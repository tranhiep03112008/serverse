...
let signName = function(client, name){
	if (!!name) {
		...
		if (!validator.isLength(name, {min: 6, max: 16})) {
			client.red({notice: {title: 'TÊN NHÂN VẬT', text: 'Độ dài từ 6 đến 16 ký tự !!'}});
		}else if (!testName) {
									...
									try {
										UserInfo.create({'id':client.UID, 'name':name, 'joinedOn':new Date()}, function(errC, user){
											if (!!errC) {
												...
												Message.create({'uid': client.UID, 'title':'Thành Viên Mới', 'text':'Chào mừng bạn đến với GO68 - Cổng game hàng đầu Châu Á, chúc bạn chơi game vui vẻ, thắng lớn...', 'time':new Date()});
												GameState(client);
												client.red(data);
											}
										});
