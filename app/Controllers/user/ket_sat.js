...
function gui(client, red){
	red = red>>0;
	if (red < 10000) {
		client.red({notice:{title: 'GỬI GO', text: 'Số tiền gửi phải lớn hơn 10.000'}});
	}else{
				...
				UserInfo.findOne({id: client.UID}, 'red ketSat', function(err, user){
					if(user){
						if (user.red < red) {
							...
							client.red({notice:{title:'THÀNH CÔNG', text: 'Đã gửi ' + Helper.numberWithCommas(red) + ' GO vào két sắt thành công.!!'}, user:{red:user.red-red, ketSat: user.ketSat*1+red}});
						}
					}
				});
			}else{
...
function rut(client, data){
	...
	if (red < 10000) {
		client.red({notice:{title: 'RÚT GO', text: 'Số tiền rút phải lớn hơn 10.000'}});
	}else{
		Phone.findOne({'uid':client.UID}, {}, function(err3, check){
			if (check) {
							...
							if (data_otp && data.otp == data_otp.code) {
								if (((new Date()-Date.parse(data_otp.date))/1000) > 180 || data_otp.active) {
									...
									if (user.ketSat < red) {
										...
										UserInfo.updateOne({id: client.UID}, {$inc:{red: red, ketSat: -red}}).exec();
										client.red({notice:{title:'THÀNH CÔNG', text: 'Rút thành công ' + Helper.numberWithCommas(red) + ' GO từ két sắt.!!'}, user:{red: user.red*1+red, ketSat: user.ketSat-red}});
									}
								}
							}else{
