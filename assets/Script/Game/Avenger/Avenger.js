...
var helper    = require('Helper'),
	reel      = require('Avenger_reel'),
	Line      = require('Avenger_lines'),
	gameBonus = require('Avenger_playBonus'),
	notice    = require('Notice'),
	dialog    = require('Avenger_dialog');
cc.Class({
	...
	onData: function(data) {
		...
		if (void 0 !== data.Avenger){
			this.Avenger(data.Avenger);
		}
		if (void 0 !== data.mini){
			cc.RedT.MiniPanel.onData(data.mini);
		}
		if (void 0 !== data.TopHu){
			cc.RedT.setting.topHu.data = data.TopHu;
			this.onGetHu();
		}
	...
	onChangerBet: function(event, customEventData){
		...
		let totlaLines = 25;
		if(this.Line.data !== undefined){
			totlaLines = this.Line.data.length;
		}
		this.tong.string = helper.numberWithCommas(totlaLines * helper.getOnlyNumberInString(this.bet.string));
	...
	onGetSpin: function(){
		cc.RedT.send({g:{avenger:{spin:{cuoc: helper.getOnlyNumberInString(this.bet.string), line: this.Line.data}}}});
	},
