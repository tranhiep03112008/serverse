...
var TX_Board    = require('TaiXiuBoard'),
	...
	TaiXiu_money_efScale = require('TaiXiu_money_efScale'),
...
cc.Class({
	...
	properties: {
		...
		smokeAnimation: cc.Node,
		efTai: TaiXiu_efScale,
		efTai_win: cc.Node,
		efXiu: TaiXiu_efScale,
		efXiu_win: cc.Node,
		efMoneyTai: TaiXiu_money_efScale,
		efMoneyXiu: TaiXiu_money_efScale,
		frameNan: {
			default: [],
			type:    cc.SpriteFrame
		},
	...
	init: function(obj) {
		...
		this.onDiceAnimationFinish = function(event){
			...
			if (this.isNan) {
				...
				if (this.diemSo > 10) {
					this.efTai.play()
					this.efTai_win.active = true;
				}else{
					this.efXiu.play()
					this.efXiu_win.active = true;
				}
			}
			this.diceAnimation.node.active = false;
			this.smokeAnimation.active = false;
		}
	...
	onLoad () {
		...
		this.onCuocClick = function(){
			...
			if (isNaN(bet) || bet < 1000) {
				...
				noticeComponent.text.string = 'Tiền cược phải lớn hơn 1.000 BOP';
	...
	playRongThan(){
		//this.rongThan.setAnimation(0,'Attack',false);
	},
	onEnable: function () {
	...
	onData: function(data){
		...
		if(void 0 !== data.update_time_remain){
			cc.RedT.setting.taixiu.time_remain = data.update_time_remain;
		}
		if(void 0 !== data.finish){
			if (cc.RedT.setting.taixiu.getLogs) {
				...
				if(this.isNan){
					...
					this.diaNan.position       = cc.v2(-0.749,-1.912);
					...
					if (this.node.activeInHierarchy) {
						...
						this.smokeAnimation.active = true;
					}else{
						this.onDiceAnimationFinish();
					}
				}
	...
	efStop: function(){
		this.efTai.stop();
		this.efXiu.stop();
		this.efTai_win.active = false;
		this.efXiu_win.active = false;
	},
	playTime: function(){
		void 0 !== this.timeInterval && clearInterval(this.timeInterval);
		this.timeInterval = setInterval(function() {
			if (cc.RedT.setting.taixiu.time_remain > 61) {
				...
				if (cc.RedT.setting.taixiu.time_remain < 73) {
					this.nodeKetQua.active = false;
					this.isNan && (this.diaNan.active = false);
					if(this.isNan){
						this.diaNan.active = false;
						this.dataLogs();
						this.nodeKetQua.active = true;
						if (this.diemSo > 10) {
							this.efTai.play()
				...
				if (cc.RedT.setting.taixiu.time_remain > 0) {
					...
					if(cc.RedT.setting.taixiu.getLogs){
						...
						//this.timeCuocSp.setAnimation(0,time,false);
					}
					this.timePopup.node.active && (this.timePopup.string = time)
	...
	nextRealTime: function(){
		if (cc.RedT.setting.taixiu.time_remain > 61) {
			...
			if (cc.RedT.setting.taixiu.time_remain > 0) {
				...
				this.timePopup.node.color = cc.Color.WHITE
				this.timePopup.string     = time;
				if (cc.RedT.setting.taixiu.time_remain <= 10)
					this.timeCuoc.node.color = cc.color(255, 69, 69, 255)
				else
