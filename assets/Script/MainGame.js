...
cc.Class({
	...
	onLoad: function () {
		if (cc.sys.isNative && cc.sys.isMobile) { 
			jsb.device && jsb.device.setKeepScreenOn && jsb.device.setKeepScreenOn(true); 
		}
		...
		if (void 0 === cc.RedT) {
			...
			//cc.RedT.audio.welcome = cc.RedT.audio.welcome;
		...
		cc.RedT.reconnect();
		this.TopHu.init(cc.RedT.MiniPanel);
		...
		//this.iconAvenger   = this.iconAvenger.getComponent('iconGameHu');
		if (cc.RedT.IS_LOGIN){
			cc.RedT.send({scene:'home'});
			this.header.reset();
			this.header.userName.string = cc.RedT.user.name;
			this.dialog.profile.CaNhan.login_name.string = cc.RedT.user.username;
			this.dialog.profile.CaNhan.username.string = cc.RedT.user.name;
			this.dialog.profile.CaNhan.red.string = helper.numberWithCommas(cc.RedT.user.red);
			this.dialog.profile.CaNhan.redKet.string = helper.numberWithCommas(cc.RedT.user.ketSat);
			...
			this.dialog.profile.CaNhan.ref_name.string = cc.RedT.user.ref_name;
	...
	onData: function(data){
		if (this.dataOn) {
			...
			if (void 0 !== data.TopHu){
				this.TopHu.onData(data.TopHu);
				this.dialog.DEvent.onHU(data.TopHu);
			}
			if (void 0 !== data.taixiu){
				cc.RedT.MiniPanel.TaiXiu.TX_Main.onData(data.taixiu);
				if(typeof(data.taixiu.taixiu) !== 'undefined' && typeof(data.taixiu.taixiu) !== 'undefined'){	
				}			
			}
			if (void 0 !== data.shop){
			...
			if (void 0 !== data.profile){
				...
				if (void 0 !== data.profile.level){
					this.dialog.VipPoint.level(data.profile.level);
				}
			}
			if (void 0 !== data.history){
				//this.dialog.lichSuGiaoDich.onData(data.history);
				//this.dialog.profile.LichSu.onData(data.history);
				this.dialog.profile.onData(data);
			}
	...
	dataUser: function(data){
		...
		if (void 0 !== data.username){
			this.dialog.profile.CaNhan.login_name.string = data.username;
		}
		if (void 0 !== data.red){
			this.header.userRed.string = this.dialog.profile.KetSat.redHT.string = this.dialog.profile.CaNhan.red.string = helper.numberWithCommas(data.red);
		}
		if (void 0 !== data.ketSat){
			this.dialog.profile.KetSat.redKet.string = this.dialog.profile.CaNhan.redKet.string = helper.numberWithCommas(data.ketSat);
		}
		if (void 0 !== data.ref_name){
			this.dialog.profile.CaNhan.ref_name.string = data.ref_name;
		}
		if (void 0 !== data.phone){
			...
			if (!helper.isEmpty(data.phone)) {
				...
				if (cc.RedT.user.veryphone) {
					this.dialog.profile.CaNhan.phoneStatus.string = 'Đã Xác Thực';
					this.dialog.profile.CaNhan.phoneActive.active = true;
					this.dialog.profile.CaNhan.phoneStatus.node.color  = this.dialog.profile.CaNhan.phoneStatus.node.color.fromHEX('06B30D');
					this.header.activePhone(false);
				}else{
					this.dialog.profile.CaNhan.phoneStatus.string = 'Chưa Xác Thực';
					this.dialog.profile.CaNhan.phoneActive.active = false;
					this.dialog.profile.CaNhan.phoneStatus.node.color  = this.dialog.profile.CaNhan.phoneStatus.node.color.fromHEX('FF0000');
					this.header.activePhone(false);
				}
			}else{
				this.dialog.profile.CaNhan.phoneActive.active = false;
				this.header.activePhone(true);
				//this.dialog.profile.CaNhan.phoneStatus.string = '';
			}
		}
	...
	fanpage: function(){
		cc.sys.openURL('https://gamebai.dev');
	},
	group: function(){
		cc.sys.openURL('https://gamebai.dev');
	},
	...
	onGetHu: function () {
		if (void 0 !== cc.RedT.setting.topHu.data) {
			...
			//Avenger
			// Promise.all(cc.RedT.setting.topHu.data['avenger'].filter(function (temp) {
			// 	return temp.red == true;
			// }))
			// 	.then(result => {
			// 		let h100 = result.filter(function (temp) { return temp.type == 100 });
			...
			// 		let r100 = helper.getOnlyNumberInString(this.iconAvenger.hu100.string);
			// 		let r1k = helper.getOnlyNumberInString(this.iconAvenger.hu1k.string);
			// 		let r10k = helper.getOnlyNumberInString(this.iconAvenger.hu10k.string);

			// 		if (r100 - h100[0].bet != 0) {
			// 			helper.numberTo(this.iconAvenger.hu100, helper.getOnlyNumberInString(this.iconAvenger.hu100.string), h100[0].bet, 4900, true);
			// 		}
			// 		if (r1k - h1k[0].bet != 0) {
			// 			helper.numberTo(this.iconAvenger.hu1k, helper.getOnlyNumberInString(this.iconAvenger.hu1k.string), h1k[0].bet, 4900, true);
			// 		}
			// 		if (r10k - h10k[0].bet != 0) {
			// 			helper.numberTo(this.iconAvenger.hu10k, helper.getOnlyNumberInString(this.iconAvenger.hu10k.string), h10k[0].bet, 4900, true);
			// 		}
			// 	});
			// Long Lan
